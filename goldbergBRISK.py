...
import numpy as np
from definitios import crop_image, compute_mean_level, compute_distance, compute_similarity
import pdb
...
def generate_BRISKsignature(img_path):
    ...
    # create BRISK feature extractor
    brisk = cv2.BRISK_create()

    # detect features from the image
    keypoints, descriptors = brisk.detectAndCompute(image2, None)
    ...
    for width in width_intervals:
        for length in length_intervals:
            ...
            #result.append = sorted(range(len(resp)), key = lambda sub: resp[sub])[-N:]
    ...
    for i, y in enumerate(y_coords):
        ...
        upper_x_lim = int(min([lower_x_lim + P, image.shape[1]]))
        avg_grey[i] = np.mean(gray[lower_y_lim:upper_y_lim,lower_x_lim:upper_x_lim])  # no smoothing here as in the paper
        start_point = (upper_x_lim, upper_y_lim)
        end_point = (lower_x_lim,lower_y_lim)
        cv2.rectangle(image,start_point,end_point,(0,255,0),2)
        nr = nr+1

    
    cv2.imwrite("nr2/BRISK"+img_path+".jpg", image) 
    #cv2.imwrite("window_name2"+img_path+".jpg", img) 
    #pdb.set_trace()
...
sig1 = generate_BRISKsignature(path1)
sig2 = generate_BRISKsignature(path2)
sig3 = generate_BRISKsignature(path3)
sig4 = generate_BRISKsignature(path4)
